...
type result =
  | Ok of frames
  | Ng of D.ce (* Counterexamlpe at the initial state. *)
let pp_result fmt r =
  let open Z3Intf in
  match r with
  | Ok fs ->
...
let wp (hs:S.t) (frame:Z3.Expr.expr F.frame) =
  ...
  MySet.fold
    ...
    ~f:(fun l t ->
      ...
      apply_on_id (fun e -> mk_dl_and e (mk_dl_dyn srcloc.flow srcloc.inv (mk_dl_prim post_z3_pre))) srcid l)
    hs.transitions
...
let setup_init_frames ~(hs:S.t) ~(initloc:S.id) ~(init:Z3.Expr.expr) ~(safe:Z3.Expr.expr) : frames =
  ...
  (* 0-step reachability *)
  let st0 = callZ3 (fold ~init:mk_true ~f:mk_and ((apply2 mk_and) init_frame ((apply mk_not) safe_frame))) in
  (* 1-step reachability *)
  let st1 = dl_discharge (fold ~f:mk_dl_and ~init:(mk_dl_prim mk_true) ((apply2 mk_dl_and) (apply mk_dl_prim init_frame) (wp hs ((apply mk_not) safe_frame)))) in
  match st0,st1 with
  | `Unsat,`Unsat -> 
     [| init_frame;
        safe_frame;
...
let rec verify_iter ~(hs:S.t) ~(safe:Z3.Expr.expr) ~(candidates:D.ce list) ~(frames:frames) ~(iteration_num:int) =
  U.not_implemented "verify_iter"
